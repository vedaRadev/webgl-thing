...
const toBuffers = gl => 
{
	...
	return (program, bufferConfigs) => bufferConfigs.reduce((buffers, config) => 
	{
		...
		const updateData = (dstByteOffset, srcData, srcOffset, length = 0) =>
		{
			gl.bindBuffer(glBufferTarget, buffer);
			gl.bufferSubData(glBufferTarget, dstByteOffset, srcData, srcOffset, length); 
		};

		const setData = data => 
		{
			gl.bindBuffer(glBufferTarget, buffer);
			gl.bufferData(glBufferTarget, new bufferType(data), glBufferUsage);
		};
